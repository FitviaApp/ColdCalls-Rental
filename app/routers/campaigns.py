...
def _campaign_form_validation_error(
    *,
    ...
    if (
        ...
        and voice_provider != VoiceProvider.TWILIO.value
    ):
        return "AI agent campaigns currently require Twilio as the voice provider."
    ...
    if campaign_mode == CampaignMode.AI_AGENT.value and not ai_runtime_configured:
        return "Configure Twilio, OpenAI, and ElevenLabs in Settings before creating an AI agent campaign."
    ...
    *,
    ...
    if campaign.campaign_mode == CampaignMode.AI_AGENT:
        ...
        if not ai_runtime_configured:
            return "Please configure Twilio, OpenAI, and ElevenLabs credentials in Settings first"
    if not provider_configured:
